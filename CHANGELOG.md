...
- **Optional `diarize` extra:** `pyannote.audio>=4.0` installs cleanly on this Python 3.14 venv (58 additive packages, no upgrades to existing pins) for audio-based speaker counting. Not wired into the pipeline yet — docs/13 §8 records what is verified (MVSEP API end to end, HF token + component models, measured diarization) and what integration still needs.
