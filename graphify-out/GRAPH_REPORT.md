...
Cohesion: 0.20
Nodes (6): AudioPlayerWidget, Path, Dedicated in-app audio player bar featuring playback controls, track metadata,…, Load an audio file into the player and prepare visualizer frames., Instant zero-gap stream switch between original and enhanced audio, preserving…, Stop playback and rewind to start.
...
### Community 5 - "exporter.py"
Cohesion: 0.08
Nodes (26): collections_abc, EnhancementProvider, harvester_analysis_enhancement_dsp, harvester_analysis_enhancement_provider, harvester_services_model_manager, logging, ConservativeDSPProvider, Deterministic Conservative DSP Provider for OmniRip M10. Generates synthetic… (+18 more)
...
Cohesion: 0.07
Nodes (36): apply_limiter(), apply_progressive_mono(), ensure_2d_audio(), match_spectral_slope(), ndarray, Digital Signal Processing (DSP) engine for OmniRip M10 audio enhancement.…, Match the residual high-frequency energy to follow the natural spectral decay…, Ensure audio is 2D array of shape (channels, samples). Returns:… (+28 more)

### Community 11 - "pathlib"
Cohesion: 0.07
Nodes (35): collections, datetime, harvester_analysis, harvester_analysis_scoring, harvester_analysis_titleclean, harvester_pipeline_phase2_hunt, harvester_services_musicbrainz, harvester_services_slskd (+27 more)
...
### Community 16 - "test_slskd_config.py"
Cohesion: 0.09
Nodes (37): Any, asyncio, harvester_ui_screens_soulseek_login, httpx, mock, MonkeyPatch, respx, secrets (+29 more)

### Community 17 - "AppConfig"
Cohesion: 0.17
Nodes (32): harvester_services_restoration, Restore a trashed original to its original location (FR-13 rollback step)., rollback(), AppConfig, _batch_target_path(), _choose_output_path(), _mutagen_parses(), polish_batch() (+24 more)
...
### Community 20 - "statemachine.py"
Cohesion: 0.21
Nodes (12): enum, is_transition_allowed(), legal_transitions(), The single source of truth for legal job-state transitions. ``State`` lives…, Return legal next states, including universal failure/cancel paths., Return whether a transition is valid without mutating any state., test_every_non_terminal_state_can_fail_or_cancel(), test_fallback_attempted_blocks_hunt_and_spectral_reentry() (+4 more)

### Community 21 - "numpy"
Cohesion: 0.07
Nodes (32): harvester_ui_screens_curation_workbench, numpy, platform, Popen, scipy, Vocal and Instrumental Stem Separation service for OmniRip. Multi-Stage Studio…, PreviewManager, EnhancementPreset (+24 more)

### Community 22 - "InteractiveScrubber"
Cohesion: 0.14
...
Cohesion: 0.11
Nodes (19): harvester_services_ffmpeg, SourceKind, FfmpegService, Any, ndarray, Path, SubprocessRegistry, Probe duration, sample rate, codec, and channels in a single ffprobe JSON pass. (+11 more)

### Community 25 - "._separate_bs_roformer"
Cohesion: 0.12
Nodes (24): ndarray, Path, apply_adaptive_spectral_gate(), apply_inversion_subtraction(), apply_vocal_harmonic_polish(), load_audio_numpy(), Apply natural vocal envelope leveling and gentle inter-phrase silence…, Smooth vocal dynamics and limit peak overs without phase degradation or robotic… (+16 more)
...
Cohesion: 0.09
Nodes (27): EnhancementPreset, EnhancementExporter, ndarray, Path, Generate default output path for enhancement derivative., Decode input file, render enhanced audio, write MP3 derivative, and attach ID3…, Copy metadata tags from source and append strict provenance headers., Renders enhanced audio and exports MP3 derivatives with explicit provenance… (+19 more)
...
### Community 43 - "test_slskd.py"
Cohesion: 0.28
Nodes (9): asyncio, Path, test_download_completes_and_locates_file(), test_download_uses_legacy_object_body_for_legacy_route(), test_health_check_and_openapi_verification(), handler(), test_openapi_version_templates_normalize_and_pick_0_26_route(), test_openapi_version_templates_pick_legacy_users_route() (+1 more)
...
Cohesion: 0.15
Nodes (11): platformdirs, AppPaths, Path, Platform-aware runtime directories., All paths used by harvester runtime state. The object is pure until…, Create runtime directories and return this immutable path set., Return a deterministic per-job directory without creating it., Path (+3 more)
...
### Community 49 - "P2PCandidate"
Cohesion: 0.11
Nodes (27): random, _expected_size(), _has_spam_hint(), is_hard_filtered(), _looks_transcoded(), rank_candidates(), P2P candidate hard filters and weighted scoring (docs/06 §7-§8)., Return whether a candidate fails the Phase 2 hard filters. (+19 more)
...
Cohesion: 0.06
Nodes (57): AppPaths, argparse, ArgumentParser, copy, harvester, AcoustidConfig, _apply_environment(), BatchConfig (+49 more)
...
### Community 56 - "State"
Cohesion: 0.16
Nodes (10): RuntimeError, Apply one guarded state transition through the canonical state machine., Return an aware UTC timestamp suitable for persisted events., utc_now(), assert_transition(), IllegalTransition, Raise :class:`IllegalTransition` when the transition is not valid., Raised when a job attempts a transition not allowed by the workflow. (+2 more)
...
Cohesion: 0.29
Nodes (10): mutagen_flac, mutagen_id3, soundfile, metadata_from_probe(), Any, Deterministic MP3 (ID3v2.3) and FLAC (Vorbis + picture) tagging., Build Phase M1 metadata fallback from yt-dlp probe fields., _text() (+2 more)
...
Cohesion: 0.15
Nodes (14): App, Cycle to next dynamic color theme., cycle_theme(), Theme registry and dynamic switcher for OmniRip TUI., Register custom OmniRip palettes with the Textual app theme manager., Cycle to the next available theme, apply it to the app, and return the human-…, register_custom_themes(), ComposeResult (+6 more)
...
### Community 75 - "player.py"
Cohesion: 0.23
Nodes (10): math, rich_style, rich_text, In-app Audio Player widget with real-time spectrum, oscilloscope, interactive…, Real-time Audio Visualizer Widget for OmniRip TUI. Provides multi-mode audio…, textual, textual_message, textual_reactive (+2 more)
...
### Community 77 - "SubprocessRegistry"
Cohesion: 0.24
...
### Community 86 - ".on_button_pressed"
Cohesion: 0.20
Nodes (5): Pressed, Seek playback to an absolute timestamp in seconds., Seek forward or backward by delta_s seconds., Handle timeline click seeking from the InteractiveScrubber., Cycle visualizer display mode across all varieties.
...
### Community 98 - "._run_inference"
Cohesion: 0.21
Nodes (5): Any, ModelManager, ndarray, Path, Internal inference wrapper.
...
### Community 110 - ".play"
Cohesion: 0.25
Nodes (4): Toggle playback between playing and paused/stopped., Begin audio playback and animate visualizer., Set real-time audio filter string and restart playback seamlessly if playing., Seamlessly hot-swap ffplay process at the current elapsed time.

### Community 112 - "update_progress"
Cohesion: 0.50
Nodes (3): update_progress(), on_progress(), _ui()

### Community 170 - "test_ui_player.py"
Cohesion: 0.40
...
- **Why does `WorkbenchWidget` connect `WorkbenchWidget` to `StatusBar`, `test_ui_workbench.py`, `HarvesterApp`, `.compose`, `ModelManager`, `numpy`, `app.py`, `StemSeparator`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `AppConfig` connect `AppConfig` to `PipelineOrchestrator`, `acoustid.py`, `scanner.py`, `AcoustidService`, `analysis/restoration.py`, `pathlib`, `SubprocessRegistry`, `environment.py`, `ValidationError`, `test_slskd_config.py`, `CoverArtService`, `config.py`, `SlskdService`, `FfmpegService`, `test_phase4_spectral.py`, `slskd.py`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `AudioVisualizer` connect `AudioVisualizer` to `AudioPlayerWidget`, `.load_audio_frames`, `.render`, `.compose`, `CurationWorkbenchModal`, `player.py`, `.__init__`, `InteractiveScrubber`, `app.py`, `test_ui_visualizer.py`?**
